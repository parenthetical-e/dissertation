...
This introduction is of four parts.  First I discuss ``classical'' rewards and the brain.  Second I begin making a case for cognitive rewards, discussing as an example novelty, then moving onto other examples and finally arguing for the utility of flexible (synonymous here with categorical or generalizable or inferable) reward representations in the cognitive case as compared to simple exemplar representations (e.g. say a sip of raspberry juice taken after a long run on the afternoon of July the 2nd 1982 compared to the more general category of ``drinking fruit juice'') .  Third is a discussion of prior studies of instrumental generalization (in pigeons) and multi-valued decision making (in humans) - both of which can be viewed as evidence for categorical reward representations.  Fourth, I outline the specific goals and methods of this work.  Fifth and finally is a diversion discussing other (non-rewarding) theories of phasic dopamine.

\subsection{The classics.} % (fold)
...
Classically rewards and reinforcers have been linked to (or simply were) food \cite{ODoherty:2006p2875}, pain \cite{Becerra:2011p7581,schultz:2007aa} and presumably sex though for, err, logistical reasons this is not often used in the laboratory; however physical contact with a loved one has been shown to activate the reward circuitry \cite{Izuma:2008p2822,Fliessbach:2007gf}).  These rewards are certainly potent, being used for over 50 successful years in studying learning in animal models \cite{iversen:2007aa} and people \cite{Kim:2010p7248,Montague:2006mz}.  However these kinds of rewards are not the only event that can lead to phasic firing the in the VTA/SNc accompanied by activity changes in the striatum (i.e. activation of the reward circuitry).  Novelty is an example.
